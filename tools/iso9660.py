...
class ISO9660(object):
    def __init__(self, url):
        ...
        self._url = url
        self._get_sector = self._get_sector_url if url.startswith(
            'http') else self._get_sector_file
    ...
    def _tree_path(self, name, index):
        def spacer(s): return "%s/%s" % (name, s)
    ...
    def _tree_node(self, node):
        def spacer(s): return "%s/%s" % (node['name'], s)
    ...
    def _unpack_string(self, l):
        return self._buff.read(l).rstrip(b' ').decode('utf-8')
